...
# ~~~~~~~~~~~~~~~~~~~~~
# Parametros c/ ordem 
# de atualizacao irrelevante
# ~~~~~~~~~~~~~~~~~~~~~
tmp_paramnames = c("beta","tau2","Delta")
ntype = length(tmp_paramnames)
...
for(i in 2:Q){
 ...
 tmp_param = list(beta = beta.samp[i-1,,1:Gplus.samp[i]],
                  tau2 = tau2.samp[i-1,1:Gplus.samp[i]],
                  Delta = Delta.samp[i-1,1:Gplus.samp[i]])
 
 # ---------------------------
 # selecionando aleatoriamente 
 # a ordem de atualizacao
 # ---------------------------
 tmp_ord = replicate(ntype,sample(1:Gplus.samp[i])) # atualizacao por componente aleatorio
 tmp_arg = sample(tmp_paramnames) # atualizacao por tipo de parametro aleatorio
 colnames(tmp_ord) = tmp_arg 
 
 
 for(type in 1:ntype){ # para cada tipo de parametro
  for(j in 1:Gplus.samp[i]){ # para cada componente
   # atualiza um parametro por vez da respectiva condicional completa
   # na ordem aleatoria determinada acima
   tmp_param = updateGibbs(tmp_param,tmp_arg[type],tmp_ord[j,tmp_arg[type]],
                           y,X,z.samp[i,], u.samp[i-1,], t.samp[i-1,],M,b,c,r,s,omega)
 ...
 beta.samp[i,,1:Gplus.samp[i]] = tmp_param$beta
 tau2.samp[i,1:Gplus.samp[i]] = tmp_param$tau2
 Delta.samp[i,1:Gplus.samp[i]] = tmp_param$Delta
